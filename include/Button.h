...
namespace lkankowski
{
  ...
  enum ButtonAction {
    BUTTON_NO_ACTION = 0,
    BUTTON_SINGLE_SHORT_CLICK = 0x01,
    BUTTON_DOUBLE_SHORT_CLICK = 0x02,
    BUTTON_SINGLE_LONG_CLICK = 0x03,
  };
  ...
  public:
    BounceExp()
        : Bounce(), _isExpander(false), _expanderNo(0){};

#if defined(EXPANDER_PCF8574)
    static void setExpander(PCF8574 *exp)
    {
      _expander = exp;
    };
#elif defined(EXPANDER_MCP23017)
    static void setExpander(Adafruit_MCP23017 *exp)
    {
      _expander = exp;
    };
#endif
  ...
  public:
    Button();
    void initialize(int, int, const char *, bool);
    void setAction(int, int, int);
    void attachPin(int);
    int getSensorId() { return (_sensorId); };
    bool isExposed() { return (_exposed); };
    ...
    int readState();
    int getRelayNum(int);
    int getButtonAction(bool, int);
    bool getRelayState(bool);
    int getEvent(bool, int);
    bool hasButtonActionChanged(int);
    void setButtonAction(int);
    String toString();
#if defined(EXPANDER_PCF8574)
    static void expanderInit(PCF8574 *exp)
    {
      BounceExp::setExpander(exp);
    };
#elif defined(EXPANDER_MCP23017)
    static void expanderInit(Adafruit_MCP23017 *exp)
    {
      BounceExp::setExpander(exp);
    };
#endif
